...
!=====================================================================
  ...
  subroutine get_cmt(yr,jda,ho,mi,sec,tshift_src,hdur,lat,long,depth,moment_tensor, &
                    DT,NSOURCES,min_tshift_src_original)

  !! Updated following SPECFEM v7 by Utpal Kumar 2022-12-30
...
!--- local variables below
  ...
  integer :: i,itype,istart,iend,ier, ios, iunit,mo,da
...
!
  ...
  iunit = 1
  open(unit=iunit,file=CMTSOLUTION_FILE,status='old',action='read')
...
! read source number isource
  do isource=1,NSOURCES
    ...
    ! gets header line
    read(iunit,"(a256)",iostat=ier) string
    if (ier /= 0) then
