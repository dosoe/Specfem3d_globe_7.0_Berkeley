...
!=====================================================================
  ...
  if( .not. GPU_MODE ) then
           ...
           if(USE_FORCE_POINT_SOURCE) then
              ...
              if(USE_LDDRK)then
                 stf_used = factor_force_source(isource) * &
                      comp_source_time_function_rickr(dble(it-1)*DT + dble(C_LDDRK(istage))*DT-t0-tshift_src(isource),f0)
              else
                 ! 2nd parameter is a dummy one for the ucb heaviside <SA>
                 stf_used = factor_force_source(isource) * &
                      comp_source_time_function(dble(it-1) * DT-t0-tshift_src(isource), hdur_gaussian(isource))
                 print*,'t0=',t0 !chao 2021.9.2
              ...
              if(USE_LDDRK)then
                 stf = comp_source_time_function(dble(it-1)*DT + &
                      dble(C_LDDRK(istage))*DT-t0-tshift_src(isource),hdur_gaussian(isource))
              else
     ...
     if(USE_FORCE_POINT_SOURCE) then
        do isource = 1,NSOURCES
           if(USE_LDDRK)then
              stf_pre_compute(isource) = factor_force_source(isource) * &
                   comp_source_time_function_rickr(dble(it-1)*DT + dble(C_LDDRK(istage))*DT-t0-tshift_src(isource),f0)
           else
              stf_pre_compute(isource) = &
                   factor_force_source(isource) * comp_source_time_function(dble(it-1)*DT-t0-tshift_src(isource),f0)
           endif
        enddo
     else
        do isource = 1,NSOURCES
...
!
  ...
  if( .not. GPU_MODE ) then
    ...
    do isource = 1,NSOURCES
      ...
      if(myrank == islice_selected_source(isource)) then
        ...
        if(USE_FORCE_POINT_SOURCE) then
           ...
           ! This is the expression of a Ricker; should be changed according maybe to the Par_file.
           ! stf_used = factor_force_source(isource) * comp_source_time_function_rickr(dble(NSTEP-it_tmp) &
           ! *DT-t0-tshift_src(isource),f0)

           ! e.g. we use nu_source(3,:) here if we want a source normal to the surface.
           ! note: time step is now at NSTEP-it

           ! <SA> Those 2 lines should NOT be commented for adjoint but need to adapt the code
           !      to the passage from COMPONENT_FORCE_SOURCE (now obsolete) to the reading of the
           !      FORCESOLUTION file in which the component is indicated.
          ...
          stf = comp_source_time_function(dble(NSTEP-it_tmp)*DT-t0-tshift_src(isource),hdur_gaussian(isource))
    ...
    if(USE_FORCE_POINT_SOURCE) then
      do isource = 1,NSOURCES
        stf_pre_compute(isource) = &
          factor_force_source(isource) * comp_source_time_function_rickr(dble(NSTEP-it_tmp)*DT-t0-tshift_src(isource),f0)
      enddo
    else
      do isource = 1,NSOURCES
        stf_pre_compute(isource) = &
          comp_source_time_function(dble(NSTEP-it_tmp)*DT-t0-tshift_src(isource),hdur_gaussian(isource))
      enddo
    endif
