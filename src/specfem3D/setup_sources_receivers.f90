...
!
  ...
  if(abs(minval(tshift_src)) > TINYVAL) then
     print*, '[ setup_sources ] ',abs(minval(tshift_src)),TINYVAL
  ...
  if(PRINT_SOURCE_TIME_FUNCTION .and. myrank == 0) then
     if(USE_FORCE_POINT_SOURCE) then
        do isource = 1,NSOURCES
           ...
           ! call print_stf_force_point(NSOURCES,isource,Mxx,Myy,Mzz,Mxy,Mxz,Myz, &
           !                tshift_src,hdur,min_tshift_src_original,NSTEP,DT)
           call print_stf_force_point(NSOURCES,isource, tshift_src, hdur, &
                min_tshift_src_original, NSTEP, DT)
        enddo
     else
        do isource = 1,NSOURCES
           print*, '[ICIIII]', NSTEP
        ...
        do k=1,NGLLZ
          do j=1,NGLLY
            do i=1,NGLLX
              ...
              ! <SA> We want to add the direction of the force in the global cartesian reference frame.
              ! sourcearrayd(:,i,j,k) = factor_force_source(isource) * hlagrange * &
              !      ( (/ 1., 0., 0. /) * comp_dir_vect_source_N(isource) + &
              !      (/ 0., 1., 0. /) * comp_dir_vect_source_E(isource) + &
              !      (/ 0., 0., 1. /) * comp_dir_vect_source_Z_UP(isource) ) / norm
              ! </SA>
