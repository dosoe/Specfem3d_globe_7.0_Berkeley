...
subroutine locate_sources(nspec,nglob,ibool, &
  ...
  use specfem_par,only: &
       ...
       SAVE_SOURCE_MASK, &
       factor_force_source, force_stf, & ! SA
       comp_dir_vect_source_E, comp_dir_vect_source_N, comp_dir_vect_source_Z_UP, &
       USE_FORCE_POINT_SOURCE ! SA
  ...
  if (USE_FORCE_POINT_SOURCE) then
     if (myrank == 0) then
        ! Reading FORCESOLUTION file
        call get_force(tshift_src,hdur,lat,long,depth,DT,NSOURCES,min_tshift_src_original, &
             force_stf,factor_force_source,comp_dir_vect_source_E, comp_dir_vect_source_N, &
...
! /SA
  ...
  ! Broadcasting general information about the sources to all nodes
  ...
  do isources_already_done = 0, NSOURCES, NSOURCES_SUBSET_MAX
     ...
     do isource_in_this_subset = 1,NSOURCES_SUBSET_current_size
        ...
        if(TOPOGRAPHY) then
           ...
           ! print*,'rank: ', myrank, 'says that source elevation is: ', elevation
        ...
        ! Impose source at the surface for computation of Green's functions. Not ideal <SA>
        if (USE_FORCE_POINT_SOURCE) then
           gamma = 1.d0
        endif
        ! </SA>
...
subroutine print_stf(NSOURCES,isource,Mxx,Myy,Mzz,Mxy,Mxz,Myz, &
  ...
  use specfem_par, only: USE_FORCE_POINT_SOURCE, factor_force_source
  ...
  double precision, external :: comp_source_time_function_bspline
   ...
   !   print*, scalar_moment
     ...
     if( USE_FORCE_POINT_SOURCE ) then
        ...
        write(IOUT,*) sngl(dble(it-1)*DT-t0), &
             sngl(factor_force_source(isource) * comp_source_time_function(time_source, f0))
      !   print*, sngl(dble(it-1)*DT-t0), &
      !        sngl(factor_force_source(isource) * comp_source_time_function(time_source, f0))
     else
...
subroutine print_stf_force_point(NSOURCES,isource, tshift_src,hdur, &
     min_tshift_src_original,NSTEP,DT)!, b_coeffs, knot_spline_loc)
  ...
  ! if( USE_FORCE_POINT_SOURCE ) t0 = t_length
  ...
  do it=1,NSTEP
     time_source = dble(it-1)*DT - t0/2 - t_cmt_used(isource)

     write(IOUT,*) sngl(dble(it-1)*DT-(t0/2) - t_cmt_used(isource)), &
          sngl(factor_force_source(isource) * &
          comp_source_time_function(time_source, hdur_gaussian(isource)))   
  enddo
  close(IOUT)
...
end subroutine print_stf_force_point
