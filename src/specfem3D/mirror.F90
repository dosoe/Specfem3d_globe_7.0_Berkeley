module mirror
  ...
  integer, parameter :: spln_order_mir = 4
  integer :: decim_fact_mir
  real, parameter :: period_mir = 3.e0
...
contains
  ...
  subroutine setup_mirror()
    ...
    call locate_receivers_mirror(myrank,DT,NSTEP,NSPEC_CRUST_MANTLE,NGLOB_CRUST_MANTLE, &
  ...
  subroutine write_mirror()
    ...
    integer :: ier, i, j, k
    ! <SA>
    ! character(len=10) :: meta_file
    ! </SA>

    if(it==it_begin)then
      if (myrank==0) print*,'it_begin_end=',it_begin, it_end
    ...
    if(decim_fact_mir==1)then
       ...
       if(mod(it-it_begin,decim_fact_mir)==decim_fact_mir-1)then
          ...
          if (myrank==0) print*,'count_rec_mir=',count_rec_mir
    ...
    !
    if (it==it_end)then
       ! <SA> Finalizing the header files with the number of records 
       ! if (nrec_local > 0)then
       !    write(meta_file,"(a,I3.3)") "meta.",myrank
       !    open(91,file=meta_file,form='formatted',status='old',position='append')
       !    write(91,*)count_rec_mir
       !    close(91)
       ! endif
       ! </SA>
       ! dump buffer
       ...
       if(myrank == 0)then
          ...
          open(35,file='mirror_src_E.dat',access='stream',form="unformatted",status='replace')
          !sevan, it seems that we do not need to record the first spline coeff,
          !and we try to use t0=0 directly. 2021.9.2
          ...
          write(35)int(spln_order_mir),sngl(t0),sngl(dt*decim_fact_mir),int(nrec_mir),int(count_rec_mir)
       endif
...
subroutine locate_receivers_mirror(myrank,DT,NSTEP,nspec,nglob,ibool, &
  ...
  ! double precision, allocatable, dimension(:) :: epidist <SA> Not used
  double precision, allocatable, dimension(:) :: x_found,y_found,z_found
  ! <SA> No need for a 2D array - causes memory issues, fixed by introducing a proc loop
  ...
  double precision, allocatable, dimension(:) :: xi_receiver_all,eta_receiver_all,gamma_receiver_all
   ! chao+0327
  double precision, allocatable, dimension(:) :: x_found_tmp, y_found_tmp, z_found_tmp
  double precision, allocatable, dimension(:) :: xi_receiver_tmp, eta_receiver_tmp, gamma_receiver_tmp
  double precision, allocatable, dimension(:) :: islice_selected_rec_tmp, ispec_selected_rec_tmp
  ...
  logical, allocatable :: inside_all(:)
  ...
  ! <SA> MAKING THE CODE SCALABLE WITH NUMBER OF MIRROR POINTS
  ! Reduce the amount of necessary points stored for each processor
  ! just the ones that are present on this proc.
  ...
  ! Allocate the future gathered array only on master
  ! Will be always re-updated in a proc loop, so we can relax how much memory we use.
  if ( myrank == 0 ) then
     allocate(ispec_selected_rec_all(nrec), &
          ...
          distmin(nrec), stat=ier)
     if( ier /= 0 ) call exit_MPI(myrank,'error allocating temporary mirror receiver arrays')

     allocate(ispec_selected_rec_tmp(nrec), &
          xi_receiver_tmp(nrec), &
          eta_receiver_tmp(nrec), &
          gamma_receiver_tmp(nrec), &
          x_found_tmp(nrec), &
          y_found_tmp(nrec), &
          z_found_tmp(nrec), &
          islice_selected_rec_tmp(nrec), stat=ier)
  ...
  do ispec=1,nspec
     do ia=1,NGNOD
        ...
        iglob = ibool(iax,iay,iaz,ispec) !chao+ used to obtain the global numbering
  ...
  ! <SA> Start of the modif on the main proc loop
  ! First each proc sends the necessary info that will be recovered afterwards by the master
  ! rank used as tag to be differentiated - not necessary but just cautious here
  if (myrank /= 0) then
     call SEND_I(ispec_selected_rec, nrec, 0, myrank)
     call SEND_DP(xi_receiver, nrec, 0, myrank)
     call SEND_DP(eta_receiver, nrec, 0, myrank)
     call SEND_DP(gamma_receiver, nrec, 0, myrank)
     call SEND_DP(final_distance, nrec, 0, myrank)
     call SEND_DP(x_found, nrec, 0, myrank)
     call SEND_DP(y_found, nrec, 0, myrank)
     call SEND_DP(z_found, nrec, 0, myrank)
  end if
  ...
  if(myrank == 0) then
     ...
     islice_selected_rec_tmp(:) = 0
     ispec_selected_rec_tmp(:)  = ispec_selected_rec(:)
     xi_receiver_tmp(:)         = xi_receiver(:)
     eta_receiver_tmp(:)        = eta_receiver(:)
     gamma_receiver_tmp(:)      = gamma_receiver(:)
     x_found_tmp(:)             = x_found(:)
     y_found_tmp(:)             = y_found(:)
     z_found_tmp(:)             = z_found(:)
     ! proc master 0
     do irec = 1,nrec
         if(        xi_receiver_tmp(irec)>=-1.and.   xi_receiver_tmp(irec)<=1&
            .and.  eta_receiver_tmp(irec)>=-1.and.  eta_receiver_tmp(irec)<=1&
            .and.gamma_receiver_tmp(irec)>=-1.and.gamma_receiver_tmp(irec)<=1)then

             ! Transform distmin into an array to compare across all procs
             inside_all(irec) = .true.
             distmin(irec)             = final_distance(irec)
             islice_selected_rec(irec) = 0
             ispec_selected_rec(irec)  = ispec_selected_rec_tmp(irec)
             xi_receiver(irec)         = xi_receiver_tmp(irec)
             eta_receiver(irec)        = eta_receiver_tmp(irec)
             gamma_receiver(irec)      = gamma_receiver_tmp(irec)
             x_found(irec)             = x_found_tmp(irec)
             y_found(irec)             = y_found_tmp(irec)
             z_found(irec)             = z_found_tmp(irec)
             ! increment count
     ...
     ! <SA> Have to invert the order of the loops in order not to send NPROC times the same information
     do iprocloop = 1,NPROCTOT-1
        ! Initialization of the receiving arrays
        ispec_selected_rec_all(:) = -1
        xi_receiver_all(:) = HUGEVAL
        eta_receiver_all(:) = HUGEVAL
        gamma_receiver_all(:) = HUGEVAL
        final_distance_all(:) = HUGEVAL
        x_found_all(:) = HUGEVAL
        y_found_all(:) = HUGEVAL
        z_found_all(:) = HUGEVAL

        call RECV_I(ispec_selected_rec_all, nrec, iprocloop, iprocloop)
        call RECV_DP(xi_receiver_all, nrec, iprocloop, iprocloop)
        call RECV_DP(eta_receiver_all, nrec, iprocloop, iprocloop)
        call RECV_DP(gamma_receiver_all, nrec, iprocloop, iprocloop)
        call RECV_DP(final_distance_all, nrec, iprocloop, iprocloop)
        call RECV_DP(x_found_all, nrec, iprocloop, iprocloop)
        call RECV_DP(y_found_all, nrec, iprocloop, iprocloop)
        call RECV_DP(z_found_all, nrec, iprocloop, iprocloop)
        
        do irec = 1,nrec
           if(      xi_receiver_all(irec)>=-1.and.   xi_receiver_all(irec)<=1&
                .and.  eta_receiver_all(irec)>=-1.and.  eta_receiver_all(irec)<=1&
                .and.gamma_receiver_all(irec)>=-1.and.gamma_receiver_all(irec)<=1)then
              
              inside_all(irec)=.true.
              ! Transform distmin into an array to compare across all procs
              ! distmin = final_distance_all(irec)
              distmin(irec) = final_distance_all(irec)
           ...
           if((.not.inside_all(irec)).and.(final_distance_all(irec) < distmin(irec))) then
              distmin(irec) = final_distance_all(irec)
              islice_selected_rec_tmp(irec) = iprocloop
              ispec_selected_rec_tmp(irec) = ispec_selected_rec_all(irec)
              xi_receiver_tmp(irec) = xi_receiver_all(irec)
              eta_receiver_tmp(irec) = eta_receiver_all(irec)
              gamma_receiver_tmp(irec) = gamma_receiver_all(irec)
              x_found_tmp(irec) = x_found_all(irec)
              y_found_tmp(irec) = y_found_all(irec)
              z_found_tmp(irec) = z_found_all(irec)
           endif

        enddo
     enddo
     do irec = 1,nrec
        if(.not.inside_all(irec)) then
           islice_selected_rec(irec) = islice_selected_rec_tmp(irec)
            ispec_selected_rec(irec) =  ispec_selected_rec_tmp(irec)
                   xi_receiver(irec) =         xi_receiver_tmp(irec)
                  eta_receiver(irec) =        eta_receiver_tmp(irec)
                gamma_receiver(irec) =      gamma_receiver_tmp(irec)
     ...
     !
     if(any(distmin(:) == HUGEVAL))then
        print*,nrec,ispec
        call exit_MPI(myrank,'error locating mirror receiver')
     endif
     ! final_distance_max = maxval(final_dist(:))
     final_distance_max = maxval(distmin(:))
  ...
  ! deallocate arrays 
  ! <SA> DEBUG for scalability and increasing number of points
  ...
  if (myrank == 0) then
     ...
     deallocate(z_found_all)
     deallocate(final_distance_all)
     deallocate(islice_selected_rec_tmp)
     deallocate(ispec_selected_rec_tmp)
     deallocate(xi_receiver_tmp)
     deallocate(eta_receiver_tmp)
     deallocate(gamma_receiver_tmp)
     deallocate(x_found_tmp)
     deallocate(y_found_tmp)
     deallocate(z_found_tmp)
  end if
...
subroutine setup_sources_receivers_intp_mir(myrank, &
  ...
  ! <SA> Writing proc headers test
  write(meta_file,"(a,I3.3)") "meta.",myrank
  open(91,file=meta_file,form='formatted',status='replace')
  write(91,*),nrec_local
  do i=1,irec_local
     write(91,*)number_receiver_global(i)
  enddo
  close(91)
  ! </SA>
...
subroutine bmn(b,m,n)
  ...
  !
  integer :: m,n,i
  real :: b((n+1)*m+1),bspln
  !
  do i = 1,(n+1)*m+1
     b(i) = bspln(0,n,sngl(sngl(i-1)/sngl((n+1)*m)*sngl(n+1)))
  enddo
  !
  return
  !=================
