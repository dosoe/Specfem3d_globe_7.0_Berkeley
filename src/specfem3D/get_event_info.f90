...
!--- input or output arguments of the subroutine below
  ...
  if(myrank == 0) then
    ...
    call get_event_info_serial(yr,jda,ho,mi,sec,event_name,tshift_src,t_shift, &
                        elat,elon,depth,mb, &
...
! by Ebru
  ...
  subroutine get_event_info_serial(yr,jda,ho,mi,sec,event_name,tshift_src,t_shift,&
