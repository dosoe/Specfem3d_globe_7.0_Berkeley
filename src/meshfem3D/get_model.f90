...
!=====================================================================
  ...
  ! ------- added <FM> ------------
  ! open(unit=22, file='1D_model', status='unknown')
  ...
  do k=1,NGLLZ
    do j=1,NGLLY
      do i=1,NGLLX
        ...
        !     write(22,"(9f10.2)") r_prem,rho,vpv,vph,vsv,vsh,eta_aniso,& 
        !                           Qkappa,Qmu
        !endif
        ! ===================================================================
...
! distinguish between single and double precision for reals
  if(CUSTOM_REAL == SIZE_REAL) then
    ...
    rhostore_test = sngl(rho)
    kappavstore_test = sngl(rho*(vpv*vpv - 4.d0*vsv*vsv/3.d0))
    kappahstore_test = sngl(rho*(vph*vph - 4.d0*vsh*vsh/3.d0))
    muvstore_test = sngl(rho*vsv*vsv)
    muhstore_test = sngl(rho*vsh*vsh)
    eta_anisostore_test = sngl(eta_aniso)

  else
    !double precision

    rhostore_test = rho
    kappavstore_test = rho*(vpv*vpv - 4.d0*vsv*vsv/3.d0)
    kappahstore_test = rho*(vph*vph - 4.d0*vsh*vsh/3.d0)
    muvstore_test = rho*vsv*vsv
    muhstore_test = rho*vsh*vsh
    eta_anisostore_test = eta_aniso
